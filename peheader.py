...
class ImageOptionalHeader():
    ...
    def SubSystem(self):
        types = {0:'未知',1:'本地',2:'Windows图形界面',3:'Windows控制台',
            5:'OS/2控制台',7:'POSIX控制台',8:'保留',9:'Windows CE图形界面'}
        try:
...
class ImageNtHeader(ImageFileHeader, ImageOptionalHeader):
    ...
    def show(self):
        print('入口点:    %08x' % int.from_bytes(self.addressOfEntryPoint,'little'))
        ...
        print('子系统:    %08x(%s)' % (int.from_bytes(self.subSystem,'little'),self.SubSystem()))
        print('区段数目:  %08x' % int.from_bytes(self.numberOfSections,'little'))
        print('日期:      %08x(%s)' % (int.from_bytes(self.timeDateStamp,'little'),self.Time()))
        print('首部大小:  %08x' % int.from_bytes(self.sizeOfHeaders,'little'))
        print('特征值:    %08x' % (int.from_bytes(self.characteristics,'little')))
        print('校验和:    %08x' % int.from_bytes(self.checkSum,'little'))
        print('可选头大小:%08x' % int.from_bytes(self.sizeOfOptionalHeader,'little'))
        print('RVA项数:   %08x' % int.from_bytes(self.numberOfRvaAndSizes,'little'))
